...
$('.filter-warp-sub-z input').focusin(function() {
    ...
    $('.filter-warp-sure-z a').eq(0).click(function() {
...
var min = 0;
var max = 0;
$('.filter-warp-sure-z a').eq(1).click(function() {
    // $('.filter-warp-sub-z input').val('');
    $(this).parent('.filter-warp-sure-z').css('display', 'none');
    min = Number($('.filter-warp-sub-z input').eq(0).val());
    max = Number($('.filter-warp-sub-z input').eq(1).val());
    if (min >= max) {
        $('.pb-z').css('display', 'block');
        $('.pb-x-z').click(function() {
            $('.pb-z').css('display', 'none');
            $('.filter-warp-sub-z input').val('');
        });
    } else {
        //根据价格筛选出符合价格的商品
...
$('.goods-list-ope-z input').click(function() {
    if ($(this).is(':checked')) {
        ...
        $('.par-diff-z').find('dl').each(function(index, value) {
            $(this).eq(index).replaceWith('<dl class="shop-duibi-z"><dt><a href="#"><img src="../images/01-f.jpg" alt=""></a></dt><dd><a href="#">麦斯威尔杰威尔发胶定型喷雾男士干胶头发持久定型造型啫喱水发蜡蓬松清香 快速定型，蓬松清香，不起白屑，</a><div class="dd-price-z"><span>¥0.01</span><a href="#">删除</a></div></dd></dl>');
